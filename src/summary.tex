  ...
  \begin{definitions}[Weight Functions]
    \begin{itemize}
      ...
      \item Even more generally (why?), we extend it to a
            \emph{linear function} $w : \ii^{n \times n} \to \ii$ by setting
            $w(A) = \sum_{ij \in E} w(ij) A_{ij}$ for all $A \in \ii^{n \times n}$.
    \end{itemize}
  \end{definitions}
\end{frame}
...
\begin{frame}
  ...
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}
      \STATE Let $k = \log n - 2$.
      \FORALL{possible choices of random values, simulataneously}
