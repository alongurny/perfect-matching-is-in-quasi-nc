...
\begin{frame}
  ...
  \begin{resolved}
    Perfect matching (decision/search) can be solved in:
    \begin{itemize}
      \item $\mathsf{Quasi-}\nc^2$
...
\begin{frame}
  ...
  \begin{exercise}
    For which $B \in \mathbb{R}^{n \times n}$ and $E$ satisfy $B \le \biadj{E}$?
  \end{exercise}
\end{frame}
...
\begin{frame}
  \begin{definitions}[Weight Functions]
    \begin{itemize}
      ...
      \item Even more generally (why?), we extend it to a function $w : \ii^{n \times
                n} \to \rr$ by setting
            $w(A) = \sum_{i, j} w(A_{ij})$ for all $A \in \ii^{n \times n}$.
    \end{itemize}
  \end{definitions}
\end{frame}
...
\begin{frame}
  \begin{lemma}
    Let $s = \text{poly}(n)$. Using $\OO(\log n)$ random bits we can
  ...
  \begin{proof}
    ...
    Choose $w(e_k) = 2^k$ and we are done.
  \end{proof}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \begin{lemma}
    ...
    Then $w_{i+1}(M_1) > w_{i+1}(M_2)$.
  \end{lemma}

  \begin{proof}
    Since $M_2$ appears in $G_{i+1}$, it has 
    the same weight as the $w_i$-minimum weight matching in $G_i$.
    That is $w_{i+1}(M_2) \le w_i(M_2)$.

    But if $w_{i+1}(M_1) = w_{i+1}(M_2)$, then $M_1$ would have been in $G^{i+1}$.
    by its definition.
  \end{proof}
\end{frame}
